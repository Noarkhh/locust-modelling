...
def run_rows(output_dir: Path, index: int, stride: int) -> None:
    ...
    print(
        f"design: {len(rows)} rows x {scenario.replicates} replicates, "
        f"{len(parameters)} parameters, model {scenario.model}"
    )
    if index >= len(rows):
        print(
            f"index {index} is past the design ({len(rows)} rows): nothing to "
            "do — the submitted array is wider than the sampled design"
        )
        return
    for row_index in range(index, len(rows), stride):
        ...
        if (evaluation_dir / "result.json").exists():
            print(f"eval-{row_index:05d}: skipped (result.json already exists)")
...
def _load_design(output_dir: Path) -> tuple[np.ndarray, list, Scenario]:
    ...
    # Samples are decoded positionally, so a design generated by a different
    # version of parameters.py would silently assign values to the wrong
    # parameters; refuse to run rather than corrupt the campaign.
    expected_header = [parameter.name for parameter in parameters]
    if header != expected_header:
        raise ValueError(
            f"samples.csv columns do not match the current parameter "
            f"definitions for model {problem_info['model']!r} — the design "
            f"was generated by a different parameters.py; re-run 'sample'. "
            f"design: {header} vs current: {expected_header}"
        )
