...
# Optional:
#   NF_TRIALS / SPIN_TRIALS  space-separated trial ids (default: the top-4 lists below)
#   VALCAMP_DIR              output campaign dir under paramsearch-runs (default: valcamp)
#   SLURM_ACCOUNT            grant to charge (default: plglscclass26-cpu; plglocustsim01 is
#                            nearly exhausted as of 2026-09-17)
#   PATCH_WIDTH              initial patch thickness [m] override (default: scenario's 0.74 m;
#                            with full_height_patch the transverse extent shrinks to keep 750/m^2)
#   JOB_PREFIX               job-name prefix (default: vc) — use a distinct one per experiment
#
...
#   SPIN_TRIALS="01048 00783 00937" VALCAMP_DIR=valcamp3 ./validation_campaign.sh spin
#                                            # second spin batch (new top-3) into valcamp3/

set -euo pipefail

SCRATCH_RUNS=/net/afscra/people/plgjakubpryc/paramsearch-runs
SLURM_DIR="${PARAMSEARCH_DIR:?}/slurm"
NF_TRIALS="${NF_TRIALS:-00756 01030 01273 00886}"
SPIN_TRIALS="${SPIN_TRIALS:-00514 00305 00457 00682}"
SLURM_ACCOUNT="${SLURM_ACCOUNT:-plglscclass26-cpu}"
ONLY_MODEL="${1:-}"
ONLY_SCALE="${2:-}"
ONLY_TRIAL="${3:-}"
...
submit() {  # model trial agents workers nodes wall
    ...
    local name="${JOB_PREFIX:-vc}-$model-$trial-$scale_label"
    local cmd
    if [ "$nodes" -eq 1 ]; then
        ...
        cmd=(sbatch --job-name="$name" --account="$SLURM_ACCOUNT" --cpus-per-task="$workers"
             --mem="${mem_gb}G" --time="$wall"
             --export=ALL,TRIAL_RESULT="$trial_result",AGENTS="$agents",WORKERS=1,WORKERS_Y="$workers",SEED=1,OUT_DIR="$out_dir",SNAPSHOT_START=0,ITERATIONS=36100,LOCUST_KEEP_SNAPSHOTS=1,LOCUST_RUN_TIMEOUT="$timeout_s"${PATCH_WIDTH:+,PATCH_WIDTH="$PATCH_WIDTH"}
             "$SLURM_DIR/large_scale.sbatch")
    else
        cmd=(sbatch --job-name="$name" --account="$SLURM_ACCOUNT" --nodes="$nodes" --time="$wall"
             --export=ALL,TRIAL_RESULT="$trial_result",AGENTS="$agents",WORKERS_Y="$workers",SEED=1,OUT_DIR="$out_dir",SNAPSHOT_START=0,ITERATIONS=36100,LOCUST_KEEP_SNAPSHOTS=1${PATCH_WIDTH:+,PATCH_WIDTH="$PATCH_WIDTH"}
